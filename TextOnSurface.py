...
#                    embossText.cylinderFace = input.selection(0).entity
            ...
            if warning_text_input:
                warning_text_input.value = "Bad Input! This might be due to invalid font name or there is not material for Cut feature"
...
class EmbossText:
    ...
    def buildTextFeatures(self):                 
        ...
        if currentSel!= None:                   
             ...
             ui.messageBox('Please select a cylinder face in advance!')

#main entry of this command dialog 	 
def TextOnSurfaceMain():
    try:
        #current selected face: user selected manually
        global currentSel
        if currentSel == None:             
        ...
        cmdDef = commandDefinitions.itemById('EmbossText')
        if not cmdDef:
            cmdDef = commandDefinitions.addButtonDefinition('EmbossText',
                    'Emboss Text on Cylinder Face',
                    'Emboss Text on Cylinder Face.',
                    './resources') # relative resource file path is specified

        onCommandCreated = EmbossTextCommandCreatedHandler()
        ...
        if ui:
            #ui.messageBox('Failed:\n{}'.format(traceback.format_exc()))
            if warning_text_input:
                warning_text_input.value = "Bad Input!"
...
def run(context):
    ...
    try:
        ...
        commandDescription = 'Design Text on Surface'
